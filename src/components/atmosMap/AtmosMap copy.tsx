  ...
  maplibre: {
    buildLayers(atmosLayer: AtmosMapLayer) {
      ...
      if (atmosLayer.geometryType === "vector") {
        ...
        const glyph = atmosLayer.glyph ?? "arrow"

        if (glyph === "arrow") {
          return buildArrowLayer()
        }

        if (glyph === "barb") {
          const barbGeo = buildBarbGeoJSON(atmosLayer.geojson)
      ...
      function buildBarbGeoJSON(fc: GeoJSONFeatureCollection) {
        const features = []

        for (const f of fc.features) {

          const speed = f.properties["wind10.speed"]
          const dir = f.properties["wind10.direction"]

          const [lon, lat] = f.geometry.coordinates

          const len = 0.3   // degrees scale (tweak later)

          const dx = len * Math.sin(dir * Math.PI / 180)
          const dy = len * Math.cos(dir * Math.PI / 180)
